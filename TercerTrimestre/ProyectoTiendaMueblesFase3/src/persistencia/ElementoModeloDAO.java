...
public class ElementoModeloDAO {
    ...
    public List<Elemento> getElementosByCodigoModelo(String codigoModelo){
        ...
        //int idModelo = modeloDAO.getId(codigoModelo);
        try{
            //ResultSet result = conn.prepareStatement("SELECT elemento.id FROM elemento_modelo JOIN elemento ON elemento_modelo.id_elemento = elemento.id WHERE elemento_modelo.id_modelo = " + idModelo).executeQuery();
